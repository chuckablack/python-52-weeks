from flask import Flask, request
from flask_cors import CORS


quokka_app = Flask(__name__)
CORS(quokka_app)

from db_apis import get_all_hosts, set_host, get_host, get_host_portscan
from db_apis import get_all_devices, set_device
from db_apis import get_all_services, set_service

from db_apis import record_portscan_data
from worker_apis import start_host_portscan


@quokka_app.route("/hosts", methods=["GET", "PUT"])
def hosts_endpoint():

    if request.method == "GET":
        return get_all_hosts()

    elif request.method == "PUT":
        hostname = request.args.get("hostname")
        if not hostname:
            return "must provide hostname on PUT", 400

        host = request.get_json()
        set_host(host)
        return {}, 204


@quokka_app.route("/devices", methods=["GET", "PUT"])
def devices_endpoint():

    if request.method == "GET":
        return get_all_devices()

    elif request.method == "PUT":
        name = request.args.get("name")
        if not name:
            return "must provide name on PUT", 400

        device = request.get_json()
        set_device(device)
        return {}, 204


@quokka_app.route("/services", methods=["GET", "PUT"])
def services_endpoint():

    if request.method == "GET":
        return get_all_services()

    elif request.method == "PUT":
        name = request.args.get("name")
        if not name:
            return "must provide name on PUT", 400

        service = request.get_json()
        set_service(service)
        return {}, 204


@quokka_app.route("/host/scan", methods=["GET", "POST"])
def host_endpoint():

    if request.method == "GET":
        hostname = request.args.get("hostname")
        token = request.args.get("token")
        if not hostname:
            return "must provide hostname to get portscan", 400
        if not token:
            return "must provide token to get portscan", 400

        host = get_host(hostname)
        if not host:
            return "host not found", 404

        return get_host_portscan(hostname, token)

    elif request.method == "POST":
        hostname = request.args.get("hostname")
        if not hostname:
            return "must provide hostname on PUT", 400

        host = get_host(hostname)
        if not host:
            return "host not found", 404

        token = start_host_portscan(hostname)
        return {"token": token}


@quokka_app.route("/worker/portscan", methods=["POST"])
def worker_portscan_endpoint():

    portscan_data = request.get_json()
    record_portscan_data(portscan_data)

    return {}, 204
